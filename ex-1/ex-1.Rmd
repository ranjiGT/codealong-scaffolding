...
of %>%
  ...
  labs(
    ...
    subtitle = "Time period: 1985 - 2019", 
    x = "Year",
    y = "Consumed beer [l]"
  )
```
...
ggplot(data = of, aes(x = year, y = visitors_total)) + 
  geom_line() +
  geom_smooth(se = FALSE) +
...
ofm <- of %>%
  mutate(group = fct_inorder(ifelse(year >= 2011, "after 9/11", "before 9/11"))) 

ggplot(data = ofm, aes(x = year, y = visitors_total, group=1)) +
  geom_boxplot() +
  labs(
