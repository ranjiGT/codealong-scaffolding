...
#Issue: the first bar is cenetred at 0 means in the negative range so it does not make any sense.
...
ggplot(gss16, aes(x = email))+
  ...
  geom_vline(
    xintercept = median(gss16$email, na.rm = T),
    linetype = "dashed", size = 1.5
  )+
  geom_vline(
    xintercept = mean(gss16$email, na.rm = T),
...
```{r 6}
gss16 %>% 
  summarise(100*mean(snap_insta, na.rm=T))
```
...
# So the raw model coefficient wrt original variant of email is this value. We can infer that, one more year of education would lead to an expected increase of time spent with emails by 21 mins given all other factors held equal. 
```
...
# We observe the error term variance is more balanced across the range of fitted response. Thus log transform helps.
```
...
#p1 + ((p2 + p2) / (p1 + p1)) # inside a grid compound plot
```
